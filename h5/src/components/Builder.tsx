...
import type { PersonaData, WizardSel } from "../api/types";
import { colorOf } from "../lib/palette";

type Props = {
  persona: PersonaData;
  onGenerate: (sel: WizardSel, count: number, promptOverride?: string) => void;
};
...
export default function Builder({ persona, onGenerate }: Props) {
  ...
  const needsRefs = (sel.source ?? []).some((s) => refSources.includes(s));
    ...
    <div className="flex flex-col min-h-full">
        ...
        {isReview && (
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground mt-1.5 leading-relaxed">
              可直接在上方编辑提示词；改动会随本次生成一起提交，点「重置」可回到按参数自动拼装的版本。
            </p>
          </StepIntro>
        )}
      </div>
      ...
      {showCount && (
        <CountModal
          ...
          onConfirm={(n) => {
            setShowCount(false);
            // 用户手改过提示词就以手改版为准（promptOverride）
            onGenerate(sel, n, promptDraft ?? undefined);
          }}
        />
      )}
