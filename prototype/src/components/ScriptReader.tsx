...
export default function ScriptReader({
  ...
  const c = colorOf(script.scriptType);
    ...
    <div className="absolute inset-0 z-30 bg-background flex flex-col animate-[fade_.2s_ease]">
      <header className="flex items-center gap-3 px-4 h-14 border-b border-border shrink-0">
        ...
        <span className="font-mono text-xs text-muted-foreground ml-auto">
          NO.{String(script.n).padStart(2, "0")}
        </span>
      </header>
      ...
      <div className="flex-1 overflow-auto">
        <div className="px-5 pt-5 pb-3">
          ...
          <h1 className="font-serif font-black text-xl leading-snug mt-3">{script.title}</h1>

          <div className="mt-4 rounded-[16px] bg-muted/60 divide-y divide-border/70 text-[13px] overflow-hidden">
            <Row label="选题" value={script.topic} />
            <Row label="结构" value={script.structure} />
            <Row label="参数" value={`${script.topicType} ｜ ${script.source}`} />
            <Row label="宫格" value={`${script.grid} ＋ ${script.element}`} />
            <Row label="生成时间" value={fmt(script.createdAt)} />
          </div>

          {script.needsMaterial && (
            <div
        ...
        <div className="px-5 pb-28 pt-2">
          <div className="flex items-baseline justify-between mb-2">
            ...
            <span className="font-mono text-xs text-muted-foreground">约 {script.words} 字</span>
          </div>
      ...
      {script.breakdown && showBreakdown && (
        <BreakdownView breakdown={script.breakdown} onClose={() => setShowBreakdown(false)} />
      )}
    </div>
  );
}
...
function BreakdownView({
  ...
  breakdown: NonNullable<Script["breakdown"]>;
  onClose: () => void;
}) {
  return (
